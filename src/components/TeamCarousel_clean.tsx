...
const members: Member[] = [
  ...
  {
    id: "abu-alyth",
    name: "الخبير ابو اليث",
    role: "مدير فني - تنفيذ العمليات",
    bio: "خبير تنفيذي في تقنيات العزل والطلاءات الصناعية، يقود فرق التنفيذ ويضمن جودة النتائج في الموقع.",
...
const TeamCarousel: React.FC = () => {
  ...
  const slides = React.useMemo(() => {
    const res: Member[][] = [];
    for (let i = 0; i < members.length; i += itemsPerView) {
      res.push(members.slice(i, i + itemsPerView));
    }
    return res;
  }, [itemsPerView]);
